...
const ListingForm = ({ onSubmit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Map strictly to FastAPI Form dependencies
    ...
    if (formData.pickupInfo.trim()) {
      submitData.append('description', formData.pickupInfo); 
    }
    
    // Extract YYYY-MM-DD for the backend `date` schema
    ...
    try {
      ...
      onSubmit(newProduct); // Push live to table
      ...
      if (error.response?.status === 401 || error.message.includes('token')) {
        ...
        // FastAPI sends detailed validation errors in detail array or string
        const detail = error.response?.data?.detail;
        const msg = Array.isArray(detail) ? detail[0].msg : (detail || error.message);
        setErrorMsg(`Failed to list produce: ${msg}`);
      }
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 text-sm text-slate-700">
          ...
          <div className="flex items-start gap-4">
            <div className="flex-1">
              <input 
                ...
                className="cursor-pointer w-full text-sm text-slate-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-green-50 file:text-green-700 hover:file:bg-green-100 transition-colors file:cursor-pointer"
              />
            </div>
            
            {formData.image && (
              <div className="w-16 h-24 flex-shrink-0 border border-slate-200 rounded-md overflow-hidden bg-slate-50 shadow-sm">
                <img 
        ...
        <div>
          ...
          <input 
            type="text" required
            className="w-full p-2.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 bg-slate-50"
        ...
        <button 
          ...
          className={`cursor-pointer w-full mt-2 text-white font-bold py-3 px-4 rounded-lg transition-colors shadow-sm flex justify-center items-center ${
            isLoading ? 'bg-green-400 cursor-not-allowed' : 'bg-[#16A34A] hover:bg-green-700'
          }`}
        >
          {isLoading ? (
